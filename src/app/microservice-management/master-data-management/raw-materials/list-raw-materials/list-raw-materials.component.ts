...
import { GlobalVariable } from './list-raw-materials.global';
...
export class ListRawMaterialsComponent implements OnInit, AfterViewInit {
  ...
  async clickSearch() {
    try {
      ...
      this.gridMicroservices.instance.columnOption(7, 'sortOrder', 'desc');
      // }else {
  ...
  async onClickExport() {
    try {
      ...
      await this.common.export('Raw_Materials', this.txtInputSearch, this.myModal);
  ...
  fnDownloadTemplate() {
    try {
      window.open(environment.ip + '/download/template/Raw_Materials_Template.xlsx')
    } catch (err) {
      console.log(err)
    }
  }
