...
import {GlobalVariable} from './history-raw-materials.global';
...
@Component({
  selector: 'app-history-raw-materials',
  providers: [Request, Common, CompareService],
  templateUrl: './history-raw-materials.component.html',
  styleUrls: ['./history-raw-materials.scss'],
  encapsulation: ViewEncapsulation.None
})
export class HistoryRawMaterialsComponent implements OnInit, AfterViewInit {
  ...
  async getApiHistory() {
    try {
      ...
      if (response.resultCode === resultCodeSuccess) {
        ...
        let n = this.historyData.find(x=>x.rawMaterialName)
        let c = this.historyData.find(x=>x.code)
        this.hn = n.rawMaterialName + " (" + c.code + ")"

        this.historyData.forEach(element => {
          element.createdAt = element.createdAt ? moment(element.createdAt).format("DD/MM/YYYY HH:mm:ss") : ""
          element.expiryDate = element.expiryDate ? moment(element.expiryDate).format("DD/MM/YYYY HH:mm:ss") : ""
          element.isExpense = element.isExpense === true ? "Yes": element.isExpense === false ? "No" : null
          let mapStatus = this.statusList.find(x=>{return x.id === element.status})
          element.status = mapStatus ? mapStatus.name : ""
        });
      }
  ...
  onClickBackToView() {
    this.router.navigate(['/master-data-management','raw-materials',this.Id,'view']);
  }
