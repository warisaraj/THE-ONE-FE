...
import {GlobalVariable} from './history-finished-products.global';
...
@Component({
  selector: 'app-history-finished-products',
  providers: [Request, Common, CompareService],
  templateUrl: './history-finished-products.component.html',
  styleUrls: ['./history-finished-products.scss'],
  encapsulation: ViewEncapsulation.None
})
export class HistoryFinishedProductsComponent implements OnInit, AfterViewInit {
  ...
  async getApiHistory() {
    try {
      ...
      if (response.resultCode === resultCodeSuccess) {
        ...
        let n = this.historyData.find(x => x.finishedProductName)
        let c = this.historyData.find(x => x.code)
        this.hn = n.finishedProductName + " (" + c.code + ")"

        this.historyData.forEach(element => {
          element.createdAt = element.createdAt ? moment(element.createdAt).format("DD/MM/YYYY HH:mm:ss") : ""
          element.priceThai = element.priceThai
          element.priceInter = element.priceInter
          element.priceMiddleEast = element.priceMiddleEast
  ...
  onClickBackToView() {
    this.router.navigate(['/master-data-management','finished-products',this.Id,'view']);
  }
