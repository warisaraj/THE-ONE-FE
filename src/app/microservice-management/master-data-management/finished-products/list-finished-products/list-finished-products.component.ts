...
import { GlobalVariable } from './list-finished-products.global';
...
@Component({
  selector: 'app-list-finished-products',
  providers: [LayoutMenu, Request, Common],
  templateUrl: './list-finished-products.component.html',
})
export class ListFinishedProductsComponent implements OnInit, AfterViewInit {
  ...
  async onOkDelete() {
    try {
      ...
      let data = {
        'finishedProductId': this.getIdDelete
      };
  ...
  async onClickExport() {
    try {
      ...
      await this.common.export('Finished_Products', this.txtInputSearch, this.myModal);
  ...
  async downloadExcel() {
    let header = [{
      A: "No",
      B: "Code",
      C: "Finished Product Name",
      D: "UOM",
      E: "Updated At",
    }];
    let data: any = await this.getDataAll();
    this.exportAsExcelFile(data, header, 'Finished_Products');
  }
          ...
          return this.request.get(checkUrl.url, checkUrl.filter)
            .then(response => {
              if (response) {
                ...
                //   this.textTotal = ' Search Results 0 of 0 Item';
                //   this.goAlert(resultDescriptionDataNotFoundTitle, resultDescriptionDataNotFoundMassage, 'myModalError');
                // }else if (this.resResultCode === resultCodeDbError) {
                //   this.loading = false;
                //   this.dataMicroservices = {};
                //   this.getDataMicroservices = [];
                //   this.numMicroservices = 0;
  ...
  fnDownloadTemplate() {
    try {
      window.open(environment.ip + '/download/template/Finished_Products_Template.xlsx')
    } catch (err) {
      console.log(err)
    }
  }
