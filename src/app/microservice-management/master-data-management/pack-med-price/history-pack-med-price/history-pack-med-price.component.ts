...
import {GlobalVariable} from './history-pack-med-price.global';
...
@Component({
  selector: 'app-history-pack-med-price',
  providers: [Request, Common, CompareService],
  templateUrl: './history-pack-med-price.component.html',
  styleUrls: ['./history-pack-med-price.scss'],
  encapsulation: ViewEncapsulation.None
})
export class HistoryPackMedPriceComponent implements OnInit, AfterViewInit {
  ...
  async getApiHistory() {
    try {
      ...
      if (response.resultCode === resultCodeSuccess) {
        ...
        let n = this.historyData.find((x: { packMedPriceName: any; })=>x.packMedPriceName)
        this.packMedPriceName = n.packMedPriceName
  ...
  onClickBackToView() {
    this.router.navigate(['/master-data-management', 'pack-med-price', this.Id, 'view']);
  }
