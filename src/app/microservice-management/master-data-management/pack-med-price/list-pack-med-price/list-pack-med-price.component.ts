...
import { GlobalVariable } from './list-pack-med-price.global';
...
@Component({
  selector: 'app-list-pack-med-price',
  providers: [LayoutMenu, Request, Common],
  templateUrl: './list-pack-med-price.component.html',
})
export class ListPackMedPriceComponent implements OnInit, AfterViewInit {
  ...
  async clickSearch() {
    try {
      ...
      this.gridMicroservices.instance.columnOption(4, 'sortOrder', 'desc');
      this.dataMicroservices = await this.customStore();
      // }else {
  ...
  async onOkDelete() {
    try {
      ...
      let data = {
        'packMedPriceId': this.getIdDelete
      };
  ...
  async onClickExport() {
    try {
      ...
      await this.common.export('Pack_Med_Price', this.txtInputSearch, this.myModal);
    ...
    dataSource.store = new CustomStore({
      load: (loadOptions) => {
        if (!this.loadData) {
          ...
          if (this.common.trimData(this.txtInputSearch.packMedPriceName) != '') {
            filterData.packMedPriceName = this.common.trimData(this.txtInputSearch.packMedPriceName)
          }
  ...
  fnDownloadTemplate() {
    try {
      window.open(environment.ip + '/download/template/Pack_Med_Price_Template.xlsx')
    } catch (err) {
      console.log(err)
    }
  }
