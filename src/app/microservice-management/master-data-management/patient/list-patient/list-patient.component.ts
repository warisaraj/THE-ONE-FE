...
import { GlobalVariable } from './list-patient.global';
...
export class ListPatientComponent implements OnInit, AfterViewInit {
  ...
  async onClickExport() {
    try {
      ...
      const filter = { ...this.txtInputSearch };
      if (this.common.isRoleClinics(this.role)) {
        filter.locationType = 2;
        filter.locationDetail = this.role;
      }
      await this.common.export('Patients_Info', filter, this.myModal);
    ...
    dataSource.store = new CustomStore({
      load: (loadOptions) => {
        if (!this.loadData) {
          ...
          if (this.txtInputSearch.dateOfBirth) {
            filterData.dateOfBirth = moment(this.txtInputSearch.dateOfBirth).format('YYYY-MM-DD')
          } else {
            delete filterData.dateOfBirth
          }
            ...
            .then(response => {
              if (response) {
                ...
                if (this.resResultCode === resultCodeSuccess) {
                  ...
                  this.getDataMicroservices.forEach(element => {
                    var todayDate = moment(new Date());
                    var startDate = moment(element.dateOfBirth, 'YYYY-MM-DD')
                    var diffTime = moment(todayDate).diff(startDate);
                    var duration = moment.duration(diffTime);
                    var years = duration.years()
                    var months = duration.months()
                    var days = duration.days()

                    element.age = years + 'Y ' + months + 'M ' + days + 'D'
                    element.dateOfBirth = element.dateOfBirth ? moment(element.dateOfBirth, 'YYYY-MM-DD').format("DD/MM/YYYY") : ""
                    element.updatedAt = element.updatedAt ? moment(element.updatedAt).format("DD/MM/YYYY HH:mm:ss") : ""
                  });
  ...
  async fnDownloadTemplate() {
    try {
      ...
      const checkUrl = this.common.checkMockupUrl('', '', filter, {
        ...
        BASE_RESOURCE: environment.getPatientTemplate
      });
      const resp = await this.request.get(checkUrl.url, checkUrl.filter);
      if (resp.resultCode === environment.resultCodeSuccess) {
