...
import {GlobalVariable} from './history-price.global';
    ...
    try {
      ...
      if (response.resultCode === resultCodeSuccess) {
        ...
        this.historyData.forEach(element => {
          ...
          if(element.productionCharge){
            for (var i in element.productionCharge){
              if(element.productionCharge[i].isCalculate == 1){
                element.productionCharge[i].productionCharge = `(${element.productionCharge[i].dividend}/${element.productionCharge[i].divider})*Supply Day`
              }else{
                element.productionCharge[i].productionCharge = element.productionCharge[i].productionCharge.toString();
              }
            }
          }
        });
