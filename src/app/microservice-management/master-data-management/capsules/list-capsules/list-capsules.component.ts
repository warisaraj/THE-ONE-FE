...
import { GlobalVariable } from './list-capsules.global';
...
@Component({
  selector: 'app-list-capsules',
  providers: [LayoutMenu, Request, Common],
  templateUrl: './list-capsules.component.html',
})
export class ListCapsulesComponent implements OnInit, AfterViewInit {
  ...
  async clickSearch() {
    try {
      // console.log(this.txtInputSearch.code , this.txtInputSearch.name , this.txtInputSearch.origin, this.txtInputSearch.size);
      // if(
      //   this.txtInputSearch !== '' && this.txtInputSearch !== null && this.txtInputSearch !== 'undefined' && this.txtInputSearch !== undefined
      //   ){
      ...
      this.gridMicroservices.instance.columnOption(6, 'sortOrder', 'desc');
      // }else {
  ...
  async onClickExport() {
    try {
      ...
      await this.common.export('Capsules', this.txtInputSearch, this.myModal);
  ...
  async downloadExcel() {
    ...
    this.exportAsExcelFile(data, header, 'Capsules');
  }
  ...
  async getDataAll() {
    ...
    return data.map((r, i) => {
      return {
        no: i + 1,
        code: r.code,
        capsuleName: r.capsuleName,
        uom: r.uom,
        capsuleSize: r.capsuleSize,
        origin: r.origin,
    ...
    dataSource.store = new CustomStore({
      load: (loadOptions) => {
        if (!this.loadData) {
          ...
          if (this.common.trimData(this.txtInputSearch.code) != '') {
            filterData.code = this.common.trimData(this.txtInputSearch.code)
          }
          if (this.common.trimData(this.txtInputSearch.capsuleName) != '') {
            filterData.capsuleName = this.common.trimData(this.txtInputSearch.capsuleName)
          }
          if (this.common.trimData(this.txtInputSearch.origin) != '') {
            filterData.origin = this.common.trimData(this.txtInputSearch.origin)
          }
          if (this.common.trimData(this.txtInputSearch.capsuleSize) != '') {
            filterData.capsuleSize = this.common.trimData(this.txtInputSearch.capsuleSize)
          }
  ...
  fnDownloadTemplate() {
    try {
      window.open(environment.ip + '/download/template/Capsules_Template.xlsx')
    } catch (err) {
      console.log(err)
    }
  }
