...
import {GlobalVariable} from './history-capsules.global';
...
@Component({
  selector: 'app-history-capsules',
  providers: [Request, Common, CompareService],
  templateUrl: './history-capsules.component.html',
  styleUrls: ['./history-capsules.scss'],
  encapsulation: ViewEncapsulation.None
})
export class HistoryCapsulesComponent implements OnInit, AfterViewInit {
  ...
  async getApiHistory() {
    try {
      ...
      if (response.resultCode === resultCodeSuccess) {
        ...
        let n = this.historyData.find(x=>x.capsuleName)
        let c = this.historyData.find(x=>x.code)
        this.hn = n.capsuleName + " (" + c.code + ")"
  ...
  onClickBackToView() {
    this.router.navigate(['/master-data-management','capsules',this.Id,'view']);
  }
