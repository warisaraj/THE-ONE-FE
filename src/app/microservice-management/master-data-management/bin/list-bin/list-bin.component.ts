...
import { GlobalVariable } from './list-bin.global';
...
export class ListBinComponent implements OnInit, AfterViewInit {
  ...
  async clickSearch() {
    try {
      ...
      this.gridMicroservices.instance.columnOption(3, 'sortOrder', 'desc');
      // }else {
  ...
  async onClickExport() {
    try {
      this.isLoadingPanel = true;
      await this.common.export('BIN', { binName: this.txtInputSearch }, this.myModal);
      this.isLoadingPanel = false;
    } catch (error) {
      this.isLoadingPanel = false;
    }
          ...
          try {
            ...
            if (response) {
              ...
              if (this.resResultCode === resultCodeSuccess) {
                ...
                for (let i_1 = 0; i_1 < this.getDataMicroservices.length; i_1++) {
                  this.getDataMicroservices[i_1].updatedAt = this.common.convertDate(this.getDataMicroservices[i_1].updatedAt, 'DD/MM/YYYY HH:mm:ss');
                }
