...
import {GlobalVariable} from './history-bin.global';
...
@Component({
  selector: 'app-history-bin',
  providers: [Request, Common, CompareService],
  templateUrl: './history-bin.component.html',
  styleUrls: ['./history-bin.scss'],
  encapsulation: ViewEncapsulation.None
})
export class HistoryBinComponent implements OnInit, AfterViewInit {
  ...
  async getApiHistory() {
    try {
      ...
      if (response.resultCode === resultCodeSuccess) {
        ...
        this.hn = this.historyData[0].binName
