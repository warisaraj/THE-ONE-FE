...
import {GlobalVariable} from './consumption- report.global';
...
export class ConsumptionReportComponent implements OnInit, AfterViewInit {
  ...
  async ngAfterViewInit() {
    try {
      this.filter.createdAtFrom = moment().format('YYYY-MM-DD')
      this.filter.createdAtTo = moment().format('YYYY-MM-DD')
  ...
  clickDownload(){
    ...
    if(this.filter['createdAtFrom']&&!this.filter['createdAtTo']){
      this.filter['createdAtTo']=this.filter['createdAtFrom'];
    }else if(!this.filter['createdAtFrom']&&this.filter['createdAtTo']){
      this.filter['createdAtFrom']=this.filter['createdAtTo'];
    }
    ...
    // this.downloadExcel()
    this.common.export('Consumption', this.filter, this.myModal);
  }
