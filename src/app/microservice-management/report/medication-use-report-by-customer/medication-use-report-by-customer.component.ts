...
import {GlobalVariable} from './medication-use-report-by-customer.global';
...
@Component({
  selector: 'app-medication-use-report-by-customer',
  providers: [LayoutMenu, Request, Common],
  templateUrl: './medication-use-report-by-customer.component.html',
})
export class MedicationUseReportByCustomerComponent implements OnInit, AfterViewInit {
  ...
  async ngAfterViewInit() {
    try {
      this.filter.productionEndDateFrom = moment().format('YYYY-MM-DD')
      this.filter.productionEndDateTo = moment().format('YYYY-MM-DD')
      this.max = moment().toDate()
      this.min = moment().toDate()
      const dropdown = await this.common.searchConfig();
      this.locationList = dropdown.locationTypeList || [];
    } catch (e) {
      console.log('catch: ', e);
    }
  }
  ...
  async clickClear() {
    try {
      ...
      this.max = moment().toDate()
      this.min = moment().toDate()
      this.filter.locationType = undefined
  ...
  clickDownload(){
    ...
    if(this.filter['productionEndDateFrom']&&!this.filter['productionEndDateTo']){
      this.filter['productionEndDateTo']=this.filter['productionEndDateFrom'];
    }else if(!this.filter['productionEndDateFrom']&&this.filter['productionEndDateTo']){
      this.filter['productionEndDateFrom']=this.filter['productionEndDateTo'];
    }
    if (this.filter.locationType === 'undefined') { this.filter.locationType = undefined; }
    ...
    // this.downloadExcel()
    this.common.export('Medication_Use', this.filter, this.myModal);
  }
  ...
  onValueChanged(event: any, type: string) {
    ...
    if (type == 'productionEndDateFrom') {
      this.min = this.filter[type]=='Invalid date'?null:this.filter[type];
    } else if (type == 'productionEndDateTo') {
