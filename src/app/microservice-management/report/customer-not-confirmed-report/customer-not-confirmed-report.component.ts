...
import {GlobalVariable} from './customer-not-confirmed-report.global';
...
@Component({
  selector: 'app-customer-not-confirmed-report',
  providers: [LayoutMenu, Request, Common],
  templateUrl: './customer-not-confirmed-report.component.html',
})
export class CustomerNotNonfirmedReportComponent implements OnInit, AfterViewInit {
  ...
  async ngAfterViewInit() {
    try {
      ...
      this.orderStatusList = dropdown.orderStatus.slice(0,8) || [];
  ...
  async clickClear() {
    try {
      ...
      this.max = moment().toDate()
      this.min = moment().toDate()
      this.filter.orderStatus = undefined
      this.filter.hn = null
  ...
  clickDownload(){
    ...
    if (this.filter.orderStatus === 'undefined') { this.filter.orderStatus = undefined; }
    ...
    // this.downloadExcel()
    this.common.export('Customer_Not_confirmed', this.filter, this.myModal);
  }
