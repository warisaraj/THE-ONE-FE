...
import {GlobalVariable} from './medication-use-report-by-location.global';
...
@Component({
  selector: 'app-medication-use-report-by-location',
  providers: [LayoutMenu, Request, Common],
  templateUrl: './medication-use-report-by-location.component.html',
})
export class MedicationUseReportByLocationComponent implements OnInit, AfterViewInit {
  ...
  async ngAfterViewInit() {
    try {
      ...
      this.filter.dispensedDateFrom = moment().format('YYYY-MM-DD')
      this.filter.dispensedDateTo = moment().format('YYYY-MM-DD')
  ...
  clickDownload(){
    ...
    if(this.filter['dispensedDateFrom']&&!this.filter['dispensedDateTo']){
      this.filter['dispensedDateTo']=this.filter['dispensedDateFrom'];
    }else if(!this.filter['dispensedDateFrom']&&this.filter['dispensedDateTo']){
      this.filter['dispensedDateFrom']=this.filter['dispensedDateTo'];
    }
    ...
    // this.downloadExcel()
    this.common.export('Medication_Location', this.filter, this.myModal);
  }
