...
import { GlobalVariable } from './audit-trail-report.global';
...
export class AuditTrailReportComponent implements OnInit, AfterViewInit {
  ...
  async ngAfterViewInit() {
    try {
      ...
      this.filter.dateFrom = moment().format('YYYY-MM-DD')
  ...
  clickDownload() {
    ...
    if (this.filter.status === 'undefined') { this.filter.status = undefined; }
    let filterExport = {} as any;
    filterExport.orderDateFrom = this.filter.orderDateFrom;
    filterExport.orderDateTo = this.filter.orderDateTo;

    if (this.filter.status) {
      filterExport.status = this.orderStatusMapByName[this.filter.status]
    }
    if (this.filter.hn) {
      filterExport.hn = this.filter.hn
    }

    this.common.export('Audit_Trail', filterExport, this.myModal);
  }
