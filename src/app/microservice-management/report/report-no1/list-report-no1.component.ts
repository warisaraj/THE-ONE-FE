...
import {GlobalVariable} from './list-report-no1.global';
...
@Component({
  selector: 'app-list-report-no1',
  providers: [LayoutMenu, Request, Common],
  templateUrl: './list-report-no1.component.html',
})
export class ListReportNo1Component implements OnInit, AfterViewInit {
  @ViewChild('myModal') myModal;
  @ViewChild('nextRefilldateFrom') nextRefilldateFrom: DxDateBoxComponent;
  @ViewChild('nextRefilldateTo') nextRefilldateTo: DxDateBoxComponent;
  @ViewChild('followUpdateFrom') followUpdateFrom: DxDateBoxComponent;
  @ViewChild('followUpdateTo') followUpdateTo: DxDateBoxComponent;
  ...
  filter: any = {};
  nextMax:any=undefined;
  nextMin:any=undefined;
  followMax:any=undefined;
  followMin:any=undefined;
  constructor(
  ...
  async ngAfterViewInit() {
    try {
      this.filter.nextRefillDateFrom = moment().format('YYYY-MM-DD')
      this.filter.nextRefillDateTo = moment().format('YYYY-MM-DD')
      this.nextMax = moment().toDate()
      this.nextMin  = moment().toDate()
      this.filter.followUpDateFrom = moment().format('YYYY-MM-DD')
      this.filter.followUpDateTo = moment().format('YYYY-MM-DD')
      this.followMax = moment().toDate()
      this.followMin  = moment().toDate()
    } catch (e) {
      console.log('catch: ', e);
    }
  }
  ...
  clickDownload(){
    ...
    if(this.filter['nextRefillDateFrom']&&!this.filter['nextRefillDateTo']){
      this.filter['nextRefillDateTo']=this.filter['nextRefillDateFrom'];
    }else if(!this.filter['nextRefillDateFrom']&&this.filter['nextRefillDateTo']){
      this.filter['nextRefillDateFrom']=this.filter['nextRefillDateTo'];
    }

    if(this.filter['followUpDateFrom']&&!this.filter['followUpDateTo']){
      this.filter['followUpDateTo']=this.filter['followUpDateFrom'];
    }else if(!this.filter['followUpDateFrom']&&this.filter['followUpDateTo']){
      this.filter['followUpDateFrom']=this.filter['followUpDateTo'];
    }
    ...
    // this.downloadExcel()
    this.common.export('Report_No1', this.filter, this.myModal);
  }
  ...
  onValueChanged(event: any, type: string) {
    ...
    if (type == 'nextRefillDateFrom') {
      this.nextMin = this.filter[type]=='Invalid date'?null:this.filter[type];
    } else if (type == 'nextRefillDateTo') {
      this.nextMax = this.filter[type]=='Invalid date'?null:this.filter[type];
    }else if (type == 'followUpDateFrom') {
      this.followMin = this.filter[type]=='Invalid date'?null:this.filter[type];
    }else if (type == 'followUpDateTo') {
      this.followMax = this.filter[type]=='Invalid date'?null:this.filter[type];
    }
  }
}
